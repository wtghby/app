...
public class DotMark extends LinearLayout {
	...
	public DotMark(Context paramContext, AttributeSet paramAttributeSet,
		...
		TypedArray localTypedArray = paramContext.obtainStyledAttributes(
				paramAttributeSet, R.styleable.DotMark);
	...
	public void selectItem(int paramInt) {
		...
		for (int i = 0;; i++) {
			if (i >= this.size) {
				((ImageView) this.mImageViews.get(paramInt))
						.setBackgroundResource(R.drawable.ic_dot_noselect);
				return;
			}
			((ImageView) this.mImageViews.get(i))
					.setBackgroundResource(R.drawable.ic_dot_select);
		}
	}
}